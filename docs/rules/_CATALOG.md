...
- **Registry-only categories** — [`kafka-connect`](kafka-connect/_INDEX.md) (~92 rules), [`quarkus`](quarkus/_INDEX.md) (~6 rules, distinct from `quarkus-kafka`), and [`security`](security/_INDEX.md) (~23 rules, only `DESER_JSON_TYPE_INFO_NO_ALLOWLIST.md` has a standalone doc) ship as stub `_INDEX.md` files with rule-family summaries; the per-rule standalone docs remain a backlog item. The full didactic prose for every rule still lives inline in each `RuleId.message(...)` and is carried verbatim into the violation message.
