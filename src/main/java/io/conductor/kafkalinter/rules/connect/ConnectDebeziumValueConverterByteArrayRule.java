 ...
 * {@code value.converter} to
 * {@code org.apache.kafka.connect.converters.ByteArrayConverter}.
 *
 * <p>Every Debezium connector emits SourceRecord values as Kafka Connect
 * {@code Struct} instances containing the Debezium envelope
 * ({@code before}, {@code after}, {@code source}, {@code op}, {@code ts_ms}).
 ...
 * no change-events reach Kafka. Symmetric companion to
 * {@link ConnectDebeziumKeyConverterByteArrayRule} for the value position.
 *
 * <p>Emits one violation per offending file.
 */
public final class ConnectDebeziumValueConverterByteArrayRule implements ProjectScopedRule {
    ...
    public ConnectDebeziumValueConverterByteArrayRule(Severity severity) {
        this.severity = severity;
    }

    @Override
    public RuleId id() {
        return RuleId.CONNECT_DEBEZIUM_VALUE_CONVERTER_BYTE_ARRAY;
    }
    ...
    public List<Violation> check(ProjectContext ctx) {
        ...
        for (Map.Entry<Path, Properties> e : ctx.propertiesFiles().entrySet()) {
            ...
            String valueConverter = trimOrNull(p.getProperty(VALUE_CONVERTER));
            if (valueConverter == null) continue;
            if (!BYTE_ARRAY_CONVERTER.equals(valueConverter)) continue;

            out.add(new Violation(
                    RuleId.CONNECT_DEBEZIUM_VALUE_CONVERTER_BYTE_ARRAY, severity,
                    ctx.relativize(e.getKey()), "key:" + VALUE_CONVERTER, 0,
                    "Debezium connector " + connectorClass
                            + " sets value.converter=org.apache.kafka.connect.converters."
                            + "ByteArrayConverter — this is STRUCTURALLY INVALID. Debezium "
                            + "emits SourceRecord values as a Kafka Connect Struct (the "
                            + "Debezium envelope: before/after/source/op/ts_ms); "
                            + "ByteArrayConverter's fromConnectData() is a hard `(byte[]) "
                            + "value` cast with no Struct-handling branch — it throws "
                            + "ClassCastException on the FIRST record. The task transitions "
                            + "to FAILED, no change-events reach Kafka. There is no Debezium "
                            + "configuration, SMT chain, or downstream sink in which "
                            + "ByteArrayConverter is correct for the value position. Replace "
                            + "with a Debezium-compatible converter: "
