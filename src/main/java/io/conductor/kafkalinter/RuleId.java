...
public final class RuleId {
    ...
    public static final RuleId SCHEMA_REGISTRY_AUTO_REGISTER_TRUE = register(builder("SCHEMA_REGISTRY_AUTO_REGISTER_TRUE")
            .defaultSeverity(Severity.WARNING).confidence(Confidence.HIGH).category("kafka-clients")
            .docPath("kafka-clients/SCHEMA_REGISTRY_AUTO_REGISTER_TRUE.md")
            .message("auto.register.schemas=true on a producer — the producer registers new schema versions in Schema Registry on the fly, bypassing the compatibility-review workflow. Every code deploy can silently evolve a topic's schema without any human review, and a buggy POJO change pushes an incompatible schema to a downstream-blocking topic.")
            .tagline("`auto.register.schemas` (default `true` in the Confluent Avro/Protobuf/JSON-Schema serializers) tells the serializer to call Schema Registry's `POST /subjects/{topic}-value/versions` API the first time it sees a new version of the Java class. With the default, every producer pod has full write access to the registry; one developer adding a non-nullable field to their POJO is enough to register an incompatible new version that breaks every consumer of that topic. The setting exists for local dev convenience; in any real environment, schemas should be registered out-of-band via the registry's own CI workflow (Maven plugin, CLI, schema-registry-maven-plugin) so the change goes through review.")
            .mechanism("On the producer hot path, the Avro/Protobuf/JSON serializer maintains a cache: `subject` → `schemaId`. On a cache miss, the serializer calls `POST /subjects/{subject}/versions` with the schema body. With `auto.register.schemas=true`, the registry accepts the new version if it passes its configured compatibility check (which defaults to `BACKWARD` per-subject — but is often misconfigured as `NONE` in dev clusters, then copied to prod). Even with `BACKWARD` in effect, a new optional field is acceptable to the registry while still being a breaking change for consumers that bind to a fixed Avro reader-schema. The registration is invisible from producer logs (success path) and shows up in the registry only as a new version ID — no operator sees it until a consumer breaks.")
            .impact("Two failure modes. (1) Schema sprawl: every minor field addition creates a new schema version; the registry accumulates thousands of versions per subject; debugging which version a consumer is reading becomes archaeology. (2) Hard incidents: a developer changes a field's logical type (`int` → `long`) or makes a required field optional in a way the registry's compatibility check approves but the consumer's deserializer doesn't — and consumers start throwing `SerializationException` mid-pipeline, usually at 2 AM. Fix is to roll back the producer deploy, but the bad schema version is now permanent in the registry and contaminates every replay.")
            .whyMatters("Set `auto.register.schemas=false` on every production producer. Register schemas via the `schema-registry-maven-plugin`'s `test-compatibility` and `register` goals from CI, gated by code review. Local dev can keep `auto.register.schemas=true` against a dev registry; the rule fires on production code where the override was forgotten. Disabling auto-register turns missing-schema into a clean producer-side `RestClientException` at startup, which the deploy pipeline catches before traffic hits.")
            .build());

    public static final RuleId SCHEMA_REGISTRY_USE_LATEST_VERSION_TRUE = register(builder("SCHEMA_REGISTRY_USE_LATEST_VERSION_TRUE")
            .defaultSeverity(Severity.WARNING).confidence(Confidence.HIGH).category("kafka-clients")
            .docPath("kafka-clients/SCHEMA_REGISTRY_USE_LATEST_VERSION_TRUE.md")
            .message("use.latest.version=true on a producer — the serializer ignores the schema embedded in the Java class and looks up whatever version is currently `latest` in Schema Registry; writes can match a schema that the producer's code does not in fact implement, producing records that the producer's own consumers cannot decode.")
            .tagline("`use.latest.version` (Confluent Avro/Protobuf/JSON-Schema serializers, default `false`) is a workaround for legacy 'I don't want to maintain my code's schema' workflows. When `true`, the serializer doesn't extract the schema from the generated Java class; instead, it fetches `latest` from the registry and serializes the record against that. Whatever the producer's POJO actually contains, the wire format claims to match the registry's `latest` — which may have fields the POJO doesn't populate (nulls in required fields), or omit fields the POJO does populate (silently truncated data).")
            .mechanism("With `use.latest.version=true` and `auto.register.schemas=false` (the typical pair), the producer never registers its own schema. On each send, it fetches `latest` for `{topic}-value`, treats that as the writer's schema, and serializes the POJO into it. If `latest` was registered by someone else (a different service, a manual operator action), the producer is now writing against a schema it didn't author. Field name/type mismatches don't cause exceptions — the Avro/Protobuf serializer maps by name where it can and leaves the rest. The wire bytes claim to conform to schema-id N; consumers fetch schema N from the registry and decode against it. If the producer's POJO didn't actually fill all of N's fields, the consumer sees nulls / defaults in fields that should have data. The producer logs nothing.")
            .impact("Silent data loss + silent schema drift. The producer 'works' (no exception, no log line). Consumers see records that conform to `latest` but with unexpected nulls / defaults in fields the producer's code didn't actually emit. Downstream aggregations / joins behave correctly on the wire but produce wrong answers because the data is wrong. This is the most insidious schema-registry misconfig because every observability signal says everything is fine.")
            .whyMatters("Set `use.latest.version=false` (the default) and let the serializer extract the schema from the producer's actual generated POJO. Use the schema-registry-maven-plugin in CI to verify that every code change is compatible with the registered schemas before deploy. The only legitimate use of `use.latest.version=true` is a deliberate generic-record-passthrough pipeline (e.g. a tap or replicator that has no notion of typed records) — which should be the exception, not the default.")
            .build());

    public static final RuleId SCHEMA_REGISTRY_BASIC_AUTH_USER_INFO_LITERAL = register(builder("SCHEMA_REGISTRY_BASIC_AUTH_USER_INFO_LITERAL")
            .defaultSeverity(Severity.ERROR).confidence(Confidence.HIGH).category("security")
            .docPath("security/SCHEMA_REGISTRY_BASIC_AUTH_USER_INFO_LITERAL.md")
            .message("basic.auth.user.info contains literal username:password credentials in source — every developer with read access to this repository can read the registry credentials, every CI artifact embeds them, and every container image bakes them in. The credentials are effectively public to anyone who can `gh clone` the repo.")
            .tagline("`basic.auth.user.info` (used when `basic.auth.credentials.source=USER_INFO`) is the literal `username:password` pair the Schema Registry client sends as HTTP Basic auth. A literal value in code (not behind a `${...}` placeholder, env var, ConfigProvider, or external secrets manager) means the credentials are checked into git, baked into Maven artifacts, embedded in container images, and visible in CI build logs — none of which can be retroactively scrubbed once the secret is exposed.")
            .mechanism("The Schema Registry client extracts `basic.auth.user.info` from the Kafka client `Properties` and uses it directly to construct the `Authorization: Basic <base64(user:pass)>` header on every REST call. The literal value can come from anywhere in the source tree — a `kafka.properties`, an `application.yml`, a Java `Properties.put(...)` call, a Spring `@Value` resolved at compile time. None of those reach a secrets manager. The first time the codebase is mirrored, forked, or published as an artifact, the credentials are out — and Schema Registry credentials are typically long-lived (rotation requires coordinating across every producer/consumer in the cluster, so operators rarely rotate).")
            .impact("Anyone who can read the repository (entire engineering org for internal repos; the public Internet for accidentally-public ones) has full Schema Registry credentials. With those, they can read every schema in the cluster (which often encodes business logic and PII field names), register malicious schemas if `auto.register.schemas=true` is the default, and on some Schema Registry deployments override existing schemas. On Confluent Cloud, Schema Registry API keys also expose cluster identity for billing/quota purposes.")
            .whyMatters("Replace the literal value with a placeholder that gets resolved at runtime from a real secrets backend: `${SCHEMA_REGISTRY_USER_INFO}` (env var), `@Value(\"${schema.registry.user.info}\")` (Spring + an externalized config source), `ConfigProvider.getConfig().getValue(\"schema.registry.user.info\", String.class)` (Quarkus + Kubernetes Secret / Vault), or one of the Confluent ConfigProvider implementations (`FileConfigProvider`, `VaultConfigProvider`). Once the literal is removed from history, rotate the credentials — every commit that ever contained them is forever a leak.")
            .build());
